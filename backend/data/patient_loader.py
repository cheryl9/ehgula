...
def load_patient_data(patient_id: str) -> dict:
    ...
    the same structure as mock_patient.json so healthAgent.py
    ...
    glucose_readings = [
        {
            "time": r["recorded_at"][11:16],  # extract HH:MM from timestamp
            "value": r["value"],
            "note": r.get("note", "")
        }
        for r in glucose
    ...
    # ── Assemble into the same shape as mock_patient.json ─────
    return {
        ...
        "calendar": {
            ...
            "available_clinic_slots":  []  # extend later with real booking API
        },
